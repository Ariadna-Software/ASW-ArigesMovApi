...
router.get('/cliente/ext', function(req, res){
    ...
    var query = req.query;
    if (query.parnom && query.codclien && query.codtarif && query.codactiv) {
        articulosMysql.getArticulosClienteExt(query.parnom, query.codclien, query.codtarif, query.codactiv,  query.parpro, query.parfam, query.codigo, query.obsole, function(err, articulos) {
            if (err) {
                return res.status(500).send(err.message);
            }
            if (articulos) {
