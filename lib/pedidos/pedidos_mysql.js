...
// deleteCabPedido
// modifica la cabecera de pedido
module.exports.deleteCabPedido = function (pedido, callback) {
    deleteLineasPed(pedido, function (err, res) {
        if (err) {
            callback(err);
            return;
        }
        var sql = "DELETE FROM scaped WHERE numpedcl = ?";
        sql = mysql.format(sql, pedido.numpedcl);
        var connection = conector.getConnectionAriges();
        connection.query(sql, function (err, result) {
            if (err) {
                callback(err, null);
...
module.exports.deleteLinPedido = function (linPedido, callback) {
    var sql = "DELETE FROM sliped WHERE numpedcl = ? AND numlinea = ?";
    sql = mysql.format(sql, [linPedido.numpedcl, linPedido.numlinea]);
    var connection = conector.getConnectionAriges();
    connection.query(sql, function (err, result) {
        if (err) {
            callback(err, null);
...
var getNextNumpedcl = function (callback) {
    var sql = "SELECT MAX(numpedcl) + 1 AS nxt FROM scaped";
    var connection = conector.getConnectionAriges();
    connection.query(sql, function (err, result) {
        if (err) {
            callback(err, null);
...
var deleteLineasPed = function (pedido, callback) {
