...
module.exports.getPedidosAgente = function (codagent, callback) {
    if (codagent == "null") codagent = null;
    var sql = "SELECT"
    sql += " sc.numpedcl AS numpedcl,";
    sql += " sc.codclien AS codclien,";
    sql += " cl.codactiv AS codactiv,";
...
module.exports.getPedidos = function (callback) {
    ...
    sql += " sl.dtoline2 AS dtoline2,";
    sql += " sl.importel AS importel,";
    sql += " CONCAT(COALESCE(sc.observa01, ' '),' ',COALESCE(sc.observa02, ' '),' ',COALESCE(sc.observa03, ' '),' ',COALESCE(sc.observa04, ' '),' ',COALESCE(sc.observa05, ' ')) AS observa "
    sql += " FROM scaped AS sc";
    sql += " LEFT JOIN sclien as cl ON cl.codclien = sc.codclien";
    sql += " LEFT JOIN sagent as ag ON ag.codagent = sc.codagent";
    sql += " LEFT JOIN sliped AS sl ON sl.numpedcl = sc.numpedcl";
...
module.exports.getPedidosProveedorCliente = function (codclien, callback) {
    var pedidos = [];
    var sql = "SELECT";
    sql += " cab.*, CONCAT(COALESCE(cab.observa1, ''), ' ', COALESCE(cab.observa2, ''), ' ',";
    sql += " COALESCE(cab.observa3, ''), ' ', COALESCE(cab.observa4, ''), ' ', COALESCE(cab.observa5, '')) AS observaciones,";
    sql += " lin.*";
    sql += " FROM scappr AS cab";
    sql += " LEFT JOIN slippr AS lin ON lin.numpedpr = cab.numpedpr";
    sql += " WHERE cab.codclien = ?";
    sql = mysql.format(sql, codclien);
    var connection = conector.getConnectionAriges();
    connection.query(sql, function (err, result) {
        if (err) {
        ...
        // hay que procesar a JSON
        callback(null, fnPedidosProveedorFromDbToJson(result));
        conector.closeConnection(connection);
    });
};
...
var fnPedidosProveedorFromDbToJson = function (pedidos) {
    console.log("Pedidos Proveedor", pedidos)
    var pdJs = [];
    var cabJs = null;
    var linJs = null;
    var numpedAnt = 0;
    for (var i = 0; i < pedidos.length; i++) {
        ...
        if (numpedAnt != pedido.numpedcl) {
            ...
            cabJs = {
                numpedpr: pedido.numpedpr,
                fecpedpr: pedido.fecpedpr,
                nomprove: pedido.nomprove,
                observaciones: pedido.observaciones,
                lineas: []
            };
            numpedAnt = pedido.numpedpr;
        }
        // siempre se procesa una linea
        if (pedido.numlinea) {
            linJs = {
    ...
    console.log("PDJS Proveedor", pdJs)
    return pdJs;
};
...
module.exports.postLinPedido = function (linPedido, callback) {
    getNextNumlinea(linPedido.numpedcl, function (err, res) {
        ...
        connection.query(sql, function (err, result) {
            ...
            return callback(null, linPedido);
        });
    });
};
