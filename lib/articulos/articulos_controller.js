...
router.get('/ext', function(req, res) {
    ...
    if (query.parnom || query.parpro || query.parfam || query.codigo || query.obsole) {
        articulosMysql.getArticulosExt(query.parnom, query.parpro, query.parfam, query.codigo, query.obsole, query.rotacion ,function(err, articulos) {
            if (err) {
            ...
            if (articulos) {
                ...
                return res.status(404).send('No se han encontrado articulos con esos criterios');
            }
        });
    } else {
...
router.get('/cliente', function(req, res){
    ...
    if (query.parnom && query.codclien && query.codtarif && query.codactiv) {
        articulosMysql.getArticulosCliente(query.parnom, query.codclien, query.codtarif, query.codactiv, function(err, articulos) {
            if (err) {
                return res.status(500).send(err.message);
            }
            if (articulos) {
...
router.get('/cliente/ext', function(req, res){
    // Ahora buscamos artículos con relación a un cliente para obtener los precios que le
    // son aplicables
    var query = req.query;
    if (query.parnom && query.codclien && query.codtarif && query.codactiv) {
        articulosMysql.getArticulosClienteExt(query.parnom, query.codclien, query.codtarif, query.codactiv,  query.parpro, query.parfam, query.codigo, query.obsole, query.rotacion, function(err, articulos) {
            if (err) {
                return res.status(500).send(err.message);
            }
            if (articulos) {
...
router.get('/ext/bis', function(req, res) {
    // confirmar que se han recibido correctamente los parámetros
    var query = req.query;
    if (query.parnom || query.parpro || query.parfam || query.codigo || query.obsole) {
        articulosMysql.getArticulosExtBis(query.parnom, query.parpro, query.parfam, query.codigo, query.obsole, query.rotacion ,function(err, articulos) {
            if (err) {
                return res.status(500).send(err.message);
            }
            if (articulos) {
