...
router.get('/oferta', function (req, res) {
    ofertasMysql.getOferta(req.query.numpedcl, function (err, ofertas) {
        if (err) {
            return res.status(500).send(err.message);
        }
        res.json(ofertas)
...
router.delete('/caboferta', function (req, res) {
    ofertasMysql.deleteCabOferta(req.query.numpedcl, function (err, ofertas) {
        if (err) {
            return res.status(500).send(err.message);
        }
        return res.json(ofertas)
...
router.delete('/linoferta', function (req, res) {
    ofertasMysql.deleteLinOferta(req.query.numpedcl, req.query.numlinea, function (err, ofertas) {
        if (err) {
            return res.status(500).send(err.message);
        }
        return res.json(ofertas)
