    ...
    if (query.parnom) {
        proveedoresMysql.getProveedores(query.parnom, function(err, proveedores) {
            ...
            if (proveedores) {
                ...
                return res.status(404).send('No se han encontrado provedores con esos criterios');
            }
        });
    } else {
    ...
    if (query.codprove) {
        proveedoresMysql.getDescuentosRappeles(query.codprove, function(err, proveedores) {
            ...
            if (proveedores) {
                ...
                return res.status(404).send('No se han encontrado descuentos para este proveedor');
            }
        });
    } else {
        return res.status(400).send('Formato de la petición incorrecto');
