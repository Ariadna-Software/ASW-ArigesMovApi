...
router.get('/tipos/agente/logado', function(req, res) {
    
    var query = req.query;
    if (query.tipo &&  query.login) {
        accionesMysql.getAccionesAgente(query.tipo, query.login, query.fechaInicial, query.fechaFinal, function(err, acciones) {
            if (err) {
                return res.status(500).send(err.message);
            }
...
router.post('/', function (req, res) {
    ...
    accionesMysql.postVisita(visita, function (err, result) {
        if (err) {
            return res.status(500).send(err.message);
        }
