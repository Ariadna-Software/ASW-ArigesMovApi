...
router.get('/proveedor/cliente/:codclien', function (req, res) {
    console.log("/proveedor/cliente/:codclien");
    var codclien = req.params.codclien;
    if (codclien) {
        pedidosMysql.getPedidosProveedorCliente(codclien, function (err, pedidos) {
            if (err) {
                return res.status(500).send(err.message);
            }
            return res.json(pedidos)
