...
router.get('/tiposFormasPago', function (req, res) {
    
    var query = req.query;
    fpagoMysql.getTFormaPago(query.parnom, function (err, fpago) {
        if (err) {
            return res.status(500).send(err.message);
        }
        if (fpago) {
            ...
            return res.status(404).send('No se han encontrado tipos de formas de pago con esos criterios');
        }
    });
});
